...
login_manager.login_message = "Щоб забронювати тур увідйіть до системи"
...
def user_loader(user_id):
    with Session() as session:
        return session.query(User).where (User.id == int(user_id)).first()
...
def signup():
    ...
    if signup_form.validate_on_submit():
        ...
        with Session() as session:
            ...
            session.add(user)
            session.commit()
            flash("Ви успішну зареєструвались.")
            return redirect(url_for("login"))
